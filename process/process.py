"""
 This method enriches all of the raw tweets files stored in a source directory and stores the enriched copies in a target
directory. This just adds the fields for "most specific" location, the local time, and admin_info, from Mike's API.

Usage is: anaconda2/python2 process.py source_directory target_directory

Note that this will take 20-30s to load up before running through because the tzwhere module is not friendly. Once it loads, it 
should run fine. The enrichment may cause duplication of information if the tweets have been enriched partially before being 
stored on blob-storage, but I'm keeping it that way until we finalize where and when we enrich the tweets.
"""
...
def readFile(inpath, outpath):
	...
	with open(inpath, "rt") as inFile:
		...
		while next_n_lines:
			...
			next_n_lines = list(islice(inFile, 1024*128))
	print "Processed %d lines in: %s seconds" % (num_lines, str(time.time() - start))

###
#Shamelessly stolen from SE: https://stackoverflow.com/questions/1335392/iteration-over-list-slices
###
...
#Evaluates whether or not the coordinates provided in the tweet are specific enough to meet the requirements for what we're
#analyzing. In this case, we are looking to see if we can find admin2 from the coordinates, so if the bounding box is too
#large, we don't have a reliable answer and must throw it out.
...
#		if not ld:
#			print "c: %s, date: %s, utc_offset: %s, local_date: %s" % (str(c), date_str, str(utc_offset), str(ld))
	...
	tweet["tz_offset"] = utc_offset #called this because tweets already have a field called utc_offset
	return tweet
...
#the type of coordinate that it grabbed in ([lat, lon], type) format.
...
#	if lat < -90.: 		#Debugging relic
#		print lat1, lat2
	return [lat, lon]

#If passed correct non-None coordinates and the utc date as a datetime.datetime object,
#This will return the correct utc offset for the given coords at the time of year
#in seconds, if the tzwhere module can find the timezone. Otherwise, this will return None.
...
#			if l ==	 0.0:
#			print "zone: %s, coordinates: %s, date: $s, localize: %s" % (str(x) for x in (zone, coordinates, date, zone.localize(date))
			return l
	return None

#Returns the local time aas a datetime object if passed the utc_time (as a datetime object) and
#the utc_offset in seconds. If the utc_offset passed to it is None, will return None
def getLocalDate(utc_time, utc_offset):
	if not utc_offset == None:
...
#tz = tzwhere.tzwhere() #Faster, less accurate. Will have to change forceTZ to False from the tz.tzNameAt() call in utcOffset().
main(*sys.argv[1:])
